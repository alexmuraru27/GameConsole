...
#include <espnow.h> /* ESP-NOW multiplayer transport (NP_CMD_MP_*) */
 ...
 * (mp_session.c). MP_BEGIN/MP_END bracket the ESP-NOW mode; it never overlaps the
 * HTTP path (you either poll updates or play). See ../docu/espnow.md.
 */
 ...
 * single-consumer (handleMpService, the main loop) — a classic lock-free ring,
 * safe on the single-core NONOS SDK. A full ring drops the newest packet. */
...
static void handleMpBegin(uint16_t len)
{
    const uint8_t *p = np::payload();
...
static void handleMpService(uint16_t len)
{
    if (!s_mp_active)
    {
