...
- **Single-channel arbitration** is centralized in `buzzerRefreshOutput()`: with one PWM voice, the highest-numbered track wins — but only while it is *audibly* playing, so a paused, stopped, or resting (freq 0) higher track is skipped and the channel falls through to a lower track that has a note to sound, instead of going silent. Every state change (note advance, pause/resume/stop, mute, timbre) routes through this one function that drives (or disables) TIM3 PWM on PB5.
- **Timbre** is the pulse duty cycle (a track's `duty`, applied via `timer3Trigger(freq, duty)`): the classic one-voice trick for faking instruments. Each track keeps a sticky timbre, set with `buzzerSetTimbre(track, duty_percent)` — any value in `[BUZZER_DUTY_MIN, BUZZER_DUTY_MAX]` = 10..90 (out-of-range returns false), 50% = square (default), further from 50% = thinner/reedier. Named presets `BUZZER_TIMBRE_SQUARE/_PULSE/_THIN` and the range live in `Shared/Api/buzzer_interface.h`.
