...
```bash
make all            # Build the Apps (GameXO + TestRenderer + TestBuzzer), Bootloader, Console firmware + Shared (header check)
...
make -C Apps/TestBuzzer all   # Build only the TestBuzzer buzzer test harness
...
3. `gameConsoleInit()`: `coreInit()` (SWO, `faultsInit`, `syscallInit`, GPIO, timers, buzzer, backlight PWM) → `storageInit()` (I2C, EEPROM, settings) → `applyConsoleSettings()` (read+apply the persisted mute **and backlight brightness** *before* any boot sound) → `peripheralsInit()` (DMA, USART, ADC) → `devicesInit()` (ILI9341 display, renderer, joystick, FatFs mount) → `mpuInit()` (arm MPU confinement). The `beep_step()` progress scale runs only across `peripheralsInit`/`devicesInit` (after the mute is applied), so a muted console boots silent. Games reach the console through SVC syscalls, not a shared struct (see the Kernel subsystem).
4. `main()` then runs `init()` → loop of `update()` → `render()`

The production path is the main menu: `main()` calls `mainMenuInit()` then loops `mainMenuUpdate()` / `mainMenuRender()`. The `MainMenu/` module is a small **screen state machine** (`main_menu.c` is the orchestrator) over a shared theme/draw/input layer (`menu_common.c`): the **root** menu offers *Games* / *Settings* / *Poll Updates* / *Reboot Console*. *Games* (`game_list.c`) is the centered-hero picker — lists the `.bin` games under `Games/`, browses with up/down, and launches the highlighted one with a tap of Special Button 1; `gameLoaderLoadGame()` blocks for the game's lifetime and the picker rebuilds its surface when it returns. **Holding** Special Button 1 arms a delete (confirmed on a second screen) that removes the game's `.bin` and its paired `.pak` via `loaderDeleteGame()`. *Settings* (`settings_menu.c`) is a tree of typed settings — a Buzzer Sound toggle (persisted via `ConsoleSettings.audio_enabled`, applied to `buzzerSetMute()`), a **Display** sub-category with a Brightness slider (10–100% in 10% steps, persisted via `ConsoleSettings.brightness`, applied to `backlightSetBrightness()` — a TIM9_CH2 PWM on PA3), a **WiFi** sub-category grouping *Networks* (scan/connect) and *Server address*, and a **Firmware** sub-category grouping *Upgrade OS* (`os_update.c`, self-flash the console firmware from `Firmware/Console.bin`) and *Upgrade WiFi module* (`wifi_update.c`); the boot-time mute + brightness read-and-apply runs in `gameConsoleInit()` before the boot song so a muted console boots silent (and a dimmed one comes up dim as soon as the panel is on). *Poll Updates* (`remote_update.c`) fetches the server manifest and lists each item — a game's `.bin` and its paired `.pak` collapse into one row (downloaded together), other files stand alone — diffing each against a local `Manifests/downloaded.csv` record of the last download (NEW / UPD / UpToDate) and highlighting CRC mismatches; Special Button 1 downloads the selected item (CRC-verified) and records its new CRC. *Reboot Console* calls `gameConsoleReboot()` (`NVIC_SystemReset`) for a full restart. Controls everywhere: up/down move, Special Button 1 enters/confirms/toggles, Special Button 2 steps back a level. The renderer perf harness is an ordinary loadable game, `Apps/TestRenderer/` — an endless auto-scroller (almost no game logic) whose sprite load is **swept 50%→100%→50% of the renderer's shared sprite budget** (1050 sprites, which the renderer caps as a frame total rather than per layer; TestRenderer fills it as 3×350) by a real-time triangle wave, drawing live current/min/avg/max FPS plus the load % on screen so you can watch throughput track the load (it runs uncapped). Its tile graphics ship in `TestRenderer.pak` (generated from ASCII art by `Assets/gen_tiles.py`) and are streamed at init into both the CCM asset arena and a GAME_RAM buffer; the three 350-sprite per-layer arrays (21 KB) also live in the CCM arena.
...
│   ├── Player Name       keyboard.c    — edit ConsoleSettings.player_name (multiplayer display name)
│   ├── Display
│   │   └── Brightness       [10–100%] slider → ConsoleSettings.brightness / backlightSetBrightness (TIM9_CH2 PWM on PA3)
│   ├── WiFi
...
**Crash reporting** (`Console/Src/Kernel/crash_report.c`): the SWO decode above needs a probe, so the fault handler also persists the essentials to a small RAM `CrashReport` (game name, fault kind, PC/LR/PSR, CFSR/HFSR, MMFAR/BFAR) that survives the recoverable switch back. `crashReportBeginSession()` arms a fresh record as each game launches (so a stale fault is never misattributed); `faults.c` fills it on a CPU fault, or `game_loader.c` marks it a **hang** when the liveness deadline abandons a stuck game (no fault fires). On return the loader appends one line to `Crashes/crash.log` on the SD card (`loaderAppendCrashLog()`), and the Games picker's crash banner shows the game plus the decoded cause (`PC 0x2001A3F4 DACCVIOL`). Because games are RAM-linked, those addresses map straight onto the game's `.elf` — `tools/scripts/decode_crash.py <game.elf> <PC> [LR]` (or fed a raw `crash.log` line) wraps `arm-none-eabi-addr2line` to turn them into `file:line`, so a game crash can be debugged from the log alone, no probe.

### Games / Apps (`.bin` files, loaded at runtime from SD card)
Games live under `Apps/` — `Apps/GameXO/` is the reference game, `Apps/TestRenderer/` is the renderer benchmark, and `Apps/TestBuzzer/` is the buzzer test harness (an asset-free game that steps through every buzzer ConsoleAPI call — play/pause/resume/stop/loop/done-flag, single-voice track arbitration, and stop-all — drawing each operation's status with the console fonts and logging it to SWO on the `GAME` channel; auto-runs, SB1 skips, SB2 exits). Game binaries are loaded into RAM and run **unprivileged**, isolated from the console by the MPU. **The OS owns the game loop but runs it uncapped**: a game is not a `main()` that runs forever but three callbacks the console calls — `init` once, then `update` then `render` every frame, back to back, with the console servicing its own work (USART/network, etc.) between frames. The OS does **not** cap the frame rate; a game that wants a fixed rate paces itself with `delay()` (GameXO sleeps out each frame to hold ~60 FPS; TestRenderer runs flat out to measure throughput). For **frame-rate-independent movement**, a game scales by `getDeltaTimeUs()` — the microseconds between the previous two `update()` calls, which the OS measures from the free-running 168 MHz cycle counter (`DWT->CYCCNT`) in `game_loader.c` (first frame returns 0; clamped against a stall). TestRenderer scrolls at a constant px/sec this way, so its speed is steady even as the load sweep swings the frame rate. Each game:
...
- Calls `DECLARE_GAME_HEADER(init, update, render)` at file scope in `main.c` (places a 32-byte `GameBinaryHeader` — magic, ABI version, the two runtime stubs, the three game callbacks, and the stack-guard base — into the `.game_header` section at offset 0 of the `.bin`; the macro fills the stubs and the guard base, the game names its callbacks)
...
- **MPU** (`mpu.c`): enabled with `PRIVDEFENA` (privileged console keeps the full map). A running game gets three unprivileged regions: `GAME_RAM` (RWX), the CCM asset arena (RW, execute-never), and a 256-byte no-access **stack-overflow guard band** just below the descending PSP (`AP_PRIV`, so it faults the unprivileged game but not the console; it overlaps `GAME_RAM` and wins as the higher-numbered region). The guard's base comes from the game header (`stack_guard`, emitted by the `.stack_guard` section in `linker/app.ld`), so an overflowing push/store traps as a recoverable MemManage fault — pinpointing the offending instruction — instead of silently clobbering `.bss`. Anything else — console RAM, peripherals, flash — traps as a recoverable MemManage fault. Regions are released on exit/crash.
...
- `docu/renderer.md` — the renderer, ground-up: the frame pipeline, the scanline compositor, and the hot-path design
...
### GPIO
Pin configuration runs once at boot in `gpio.c` (`gpioInit`) and is table-driven: each subsystem lists its pins as a `static const GpioPinConfig {port, pin, mode, af, pupd, ospeed, otype}` array and `gpioConfigurePin()` applies one pin's MODER/OSPEEDR/PUPDR/OTYPER/AFR with a read-modify-write that touches only that pin. `gpio.h` also exposes single-pin runtime I/O — `gpioSetPin` / `gpioClearPin` / `gpioWritePin` / `gpioReadPin` / `gpioSetPinMode` — plus the `GPIO_MODE_*` field values and named pin constants (`GPIO_I2C_SCL`, `GPIO_DISPLAY_RST`, `GPIO_SD_DETECT`, `GPIO_ESP_EN`/`RST`/`IO0`/`IO2`), so a driver drives or reads a signal by name instead of writing `BSRR`/`IDR`/`MODER` masks: the ILI9341 reset, `sdCardPresent` (`sdio.c`), the ESP-01 EN/RST/IO0 bootstrap (`esp01.c`), and the I2C bus-recovery bit-bang (`i2c.c`) all route through it. Each pin number is defined once — the named constants are used by both the config tables and the accessor call sites. A signal's *meaning* stays in its owning driver; only the register mechanics live in `gpio.c`. The joystick is the one exception: it reads its ten button pins as a single bulk `~GPIOA/B->IDR` latch per poll (see Joystick).

### SD Card Stack
Four layers: **FatFs** (`ff.c`) → **disk I/O** (`diskio.c`) → **low-level driver** (`diskio_integration.c`) → **SDIO peripheral** (`sdio.c`). `sdInit()` sends CMD0/CMD8/ACMD41/CMD2/CMD3/CMD7/CMD16, switches to 4-bit bus mode, and sets the transfer clock to 2 MHz (`sdioRaiseClock`, conservative — cards spec ≥25 MHz; both 1 MHz and 4 MHz were tried and didn't resolve the sustained-write corruption seen on this board, which is a board-level power/signal-integrity limit of the V2 PCB — a 2-layer board with no ground plane and only a 100 nF cap at the microSD VDD, no bulk capacitance, so the card browns out during write-current transients; the fix is the next PCB revision, not firmware). `disk_write()` is implemented (`sdWriteSingleBlock`/`sdWriteMultipleBlocks`) and **reads every block back, comparing it to the source and retrying up to 3× before failing** (`verifyWrite` in `diskio.c`), so a flaky card can't silently store wrong bytes. FatFs is built read-write (`FF_FS_READONLY 0`), so deletes/writes work — e.g. a download whose CRC doesn't match the manifest removes the partial file. Card detect is on GPIO PD3 (active-low, `sdCardPresent()` in `gpio.c`). Content is organized into directories — created (only if missing) at every mount by `ensureDirs()` in `loader.c`, names in `Inc/sd_layout.h`: `Games/` (game `.bin` + `.pak`), `Settings/` (`server.txt`), `Firmware/` (`ESP01.bin`), `Manifests/` (the fetched remote `manifest.csv` + the local `downloaded.csv` diff record), `Crashes/` (`crash.log`, one appended line per game crash/hang).
...
- **Hot-path tricks**: an opaque two-pixels-per-store blitter, a packed `s_pair` palette LUT, a per-frame decoded-tile cache, and a "byte is fully opaque" table keep a busy screen fast (see `docu/renderer.md`).
- **Public API**: `rendererClear` / `rendererSetBackground` / `rendererSubmitLayer` / `rendererDrawText` / `rendererRender`, plus `rendererGetWidthPixels` / `rendererGetHeightPixels` and `rendererSystemColor` (Pixel Forge system-palette index → RGB565). This frame-submission surface is exposed to games through the `ConsoleAPI`; the `Sprite`/`Layer`/`SpriteFlags` types live in `Shared/Api/renderer_interface.h`.
- **Text** (`rendererDrawText(layer, x, y, z, font, scale, color, text)`): the console expands a whole string into glyph `Sprite`s on its own scratch pool (drawn from the built-in fonts, tinted one RGB565 colour), which fold into the layer's z-sort alongside the game's submitted sprites — so a game draws text in **one syscall** instead of building a sprite-per-glyph array and paying a `fontGet`/`fontScale` trap per glyph per frame. Scaled glyph bitmaps are cached console-side across frames; scale is clamped to `RENDERER_TEXT_MAX_SCALE`. Re-issued every frame like sprites (cleared each `rendererRender`/`rendererClear`). Alignment stays caller-side (the pen advances `(fontGlyphW(font)+1)*scale` per char). It is the one text path everywhere: all three apps (GameXO, TestRenderer, TestBuzzer) keep no text sprite array or scaled-glyph pool of their own, and the console menus route through it too — `menu_common`'s `menuDrawText`/scaled-title helpers call it directly (console-side, no SVC), leaving only the bars/underlines in `g_menu_ui` (text at z=1 over bars at z=0). Two functions are **console-only** (not game syscalls): `rendererInit` builds the compositor tables once at boot (`devicesInit`), and `rendererResetState` (drop layers + disable background) is called by the kernel when it launches a game — so a game starts from a clean slate and never inherits the menu's background if it forgets to set its own. The renderer is compiled `-O3` even in debug builds (`#pragma GCC optimize`) since it is the per-frame hot path.
- **RAM cost**: the scanline buffers + per-chunk bins + z-sort pool + tile cache + the text glyph pool/scaled-glyph cache dominate `CONSOLE_RAM` (~45 KB); `tools/memory_analysis` reports the live budget.

### Joystick
Two analog joysticks with digital d-pads + 2 special buttons. Polled by TIM7 ISR every 10ms. Digital buttons: 5ms debounce on each of 10 GPIO pins (active-low with pull-ups). Analog axes: ADC1 channels 10–13 (PC0–PC3) in DMA circular mode, read straight from the DMA buffer at latch time and centered/deadzoned to a signed −512..+512.

**Input is batched — one snapshot per frame, for both games and the console.** The driver exposes exactly `joystickPollFrame()` (latch a frame) + `joystickGetState(InputState *out)` (read it); there are no per-button getters. `InputState` (`Shared/Api/joystick_interface.h`) holds one `InputButtonState { held; pressed; released; }` per button — `r_up/r_right/r_down/r_left`, `l_up/l_right/l_down/l_left`, `special1`, `special2` — so a caller reads `in.special1.pressed` with no bit math, plus four raw analog axes (`left_x/left_y/right_x/right_y`, up/right positive). A game reads it through **one syscall**, `inputGetState(InputState *out)`. The OS latches the snapshot **once per frame** at the collect seam (`joystickPollFrame()` in `gameRuntimeCollect`, `game_loader.c`), so `pressed`/`released` give btn/btnp edge semantics for free — no reinvented debounce. The console menus latch the same way inside `menuPollNav()` / `kbPoll()` (edge-fire + typematic auto-repeat, taking button edges from the per-button `pressed` flags so a button held across a game→menu transition never re-fires), and threshold the raw axes for stick navigation.

### Buzzer
5-track software synthesizer (`buzzer.c`). TIM6 fires at 1ms, advances each active track's note counter. Notes are interleaved `uint16` pairs: `{frequency_hz, duration_ms}…`; frequency 0 = pause. Supports play with optional done-flag, pause, resume, stop, loop. Frequency constants from C2 (65 Hz) through B8 (7902 Hz) defined in `buzzer.h`.
- **Single-channel arbitration** is centralized in `buzzerRefreshOutput()`: with one PWM voice, the highest-numbered track wins — but only while it is *audibly* playing, so a paused, stopped, or resting (freq 0) higher track is skipped and the channel falls through to a lower track that has a note to sound, instead of going silent. Every state change (note advance, pause/resume/stop, mute) routes through this one function that drives (or disables) TIM3 PWM on PB5 as a fixed 50% square.

### RNG
The STM32F407's hardware true-random-number generator (`rng.c`). Its AHB2 bus clock and the 48 MHz PLLQ that feeds its analog entropy source are already configured in `systemClockConfig()` (shared with SDIO/USB); `rngInit()` (in `peripheralsInit`) just sets `RNG_CR_RNGEN`. `rngGetRandom()` spins briefly for `RNG_SR_DRDY` and returns `RNG->DR`, self-recovering from a latched seed/clock error (clear the status, restart the generator) and bounding the spin with a timeout that falls back to the hashed cycle counter so a game syscall can never wedge the console. Exposed to games as the seed-free `getRandom()` syscall (`uint32_t`).

### EEPROM Settings
AT24C512 (64KB) on I2C1 at 400kHz. A CRC-16 protected key/value store, packed flat with no arbitrary padding, split across three files: `settings_storage.c` (the public API — console blob, keyed game saves, management, current-game binding — plus the in-RAM system header), `settings_directory.c` (the directory/slot mechanism: address math, CRC, raw EEPROM I/O, entry read/write/occupancy/find/free/count, and the shared 2 KB entity codec), and the private `settings_layout.h` (the on-EEPROM layout: the packed structs and the region addresses):
...
- **Display**: ILI9341 240×320 via FSMC 16-bit 8080 parallel (PD/PE pins). Rotation 1 = landscape. Backlight on PA3 (TIM9_CH2 PWM, brightness-controlled), reset on PC7.
