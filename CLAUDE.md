...
`GameXO/` is the reference game. Game binaries are loaded into RAM and run **unprivileged**, isolated from the console by the MPU. **The OS owns the game loop**: a game is not a `main()` that runs forever but three callbacks the console calls — `init` once, then `update` then `render` every frame, with the console pacing the frame and servicing its own work (USART/network, etc.) in between. Each game:
