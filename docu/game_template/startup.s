 ...
 ******************************************************************************
 * @file      startup.s
 * @brief     Game entry + binary header — TEMPLATE. Copy this verbatim into a
 *            new game; it is identical for every game.
 *
 *            It emits the 12-byte GameBinaryHeader at offset 0 of the .bin and
 *            defines _game_start, the entry trampoline. The console loader
 *            validates the header, copies the flat image to GAME_RAM, and (via
 *            the kernel) enters _game_start unprivileged on a fresh PSP.
 *            _game_start zeroes .bss, runs static constructors, calls main(),
 *            then traps back to the console with gameExit().
 *
 *            There is NO vector table — the console owns every interrupt and
 *            exception; a game only ever traps out through SVC.
 *
 *            Build note: this is assembled with `gcc -x assembler-with-cpp`, so
 *            the #include and the GAME_BINARY_MAGIC / CONSOLE_ABI_VERSION defines
 *            resolve through the C preprocessor. Make sure the assembler flags
 *            carry `-I<repo>/Shared/Api` (and Shared/Syscall).
 ******************************************************************************
 */
