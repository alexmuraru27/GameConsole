...
produced by an originator (here: Timeline.snapshot()) and never inspects
...
Coalescing: a burst of related mutations (typing a duration digit by digit,
holding an arrow key to transpose) can share a coalesce_id; only the first
push of a run is recorded, so the whole burst undoes as one step.
"""
