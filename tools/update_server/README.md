...
Serving always writes a `manifest.csv` snapshot at startup and re-generates the
manifest live on every request (so the served copy is always fresh). Then from
another machine / the console:
...
- It is a LAN/dev tool: `http.server` sanitizes paths (no `..` traversal) but
  there's no auth or TLS. Don't expose it to the public internet.
